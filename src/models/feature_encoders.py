...
class FeatureEncoder(nn.Module):
    ...
    - Sentiment: VIX-based market sentiment [-1, 1] x 2
    
    Architecture:
        Time: 1 → 16 (Tanh activation)
        Wealth: 1 → 32 (Tanh activation)  
        Sentiment: 2 → 16 (Tanh activation)
    ...
    def __init__(self, time_dim: int = 16, wealth_dim: int = 32, sentiment_dim: int = 16):
        ...
        self.sentiment_encoder = nn.Sequential(
            nn.Linear(2, sentiment_dim),  # [vix_sentiment, vix_momentum]
            nn.Tanh()
        )
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            state: (batch_size, 4) tensor [time, wealth, vix_sentiment, vix_momentum]
        ...
        sentiment = state[:, 2:4]   # (batch_size, 2)
...
class AdaptiveEncoder(nn.Module):
    """
    Adaptive encoder that handles both 2D and 4D state inputs
    ...
    def __init__(self, time_dim: int = 16, wealth_dim: int = 32, sentiment_dim: int = 16):
        ...
        # Feature encoder for 4D input
        self.feature_encoder = FeatureEncoder(time_dim, wealth_dim, sentiment_dim)
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            state: (batch_size, state_dim) where state_dim ∈ {2, 4}
        ...
        if input_dim == 2:
            ...
            raise ValueError(f"Unsupported state dimensionality: {input_dim}. Expected 2 or 4.")
...
def create_encoder(
    ...
    **kwargs
    ...
    if encoder_type == "feature":
        if input_dim != 4:
            raise ValueError("FeatureEncoder requires 4D input")
        ...
        return FeatureEncoder(**feature_kwargs)
        ...
        return AttentionEncoder(input_dim=input_dim, **kwargs)
