...
class VIXProcessor:
    ...
    def get_ml_features(self, vix_df: pd.DataFrame, date: pd.Timestamp) -> np.ndarray:
        """
        ...
        Returns:
            ...
            - vix_momentum: float in [-1, 1] (5-day change normalized)
        """
        ...
        # Momentum: Use normalized 5-day change
        vix_change_5d = row['vix_change_5d']
        if pd.isna(vix_change_5d):
            vix_momentum = 0.0
        else:
            # Clip large changes and normalize to [-1, 1]
            vix_momentum = np.clip(vix_change_5d / 0.5, -1.0, 1.0)  # 50% change = full range
    ...
    def _handle_missing_values(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        change_columns = ['vix_change_1d', 'vix_change_5d', 'vix_change_20d']
